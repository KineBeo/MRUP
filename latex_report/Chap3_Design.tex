...
\begin{figure}[H]
  ...
  \begin{adjustbox}{center}
    \includegraphics[width=1.24\textwidth]{figures/fuzzing/Overview Approach MRUP Final.png}
  \end{adjustbox}
  \caption{Overview of the MRUP fuzzing approach}
\end{figure}
...
SQLite không cho phép frame specification với ROW\_NUMBER, RANK, DENSE\_RANK [https://sqlite.org/windowfunctions.html]. Lý do là các hàm này về bản chất đã hoạt động trên toàn bộ partition theo ORDER BY, việc thêm frame không có ý nghĩa. Ví dụ vi phạm:
...
SQLite không hỗ trợ RANGE frame khi ORDER BY có nhiều hơn một cột~\cite{SQLiteWindow}.
Điều này xuất phát từ độ phức tạp trong việc xác định "khoảng cách" giữa các hàng khi có nhiều cột sắp xếp. Ví dụ vi phạm:
...
Các hàm non-deterministic (RANDOM(), CURRENT\_TIMESTAMP) cho kết quả khác nhau mỗi lần chạy, gây false positives. MRUP Oracle chỉ sử dụng các hàm deterministic: ROW\_NUMBER, RANK, DENSE\_RANK, SUM, AVG, COUNT, MIN, MAX. Các hàm bị cấm bao gồm RANDOM(), CURRENT\_TIMESTAMP, \_\allowbreak CURRENT\_DATE, 
CURRENT\_\allowbreak TIME
, và các UDF không deterministic.
...
\textbf{Nguyên tắc:}
\begin{itemize}
    \item \ding{51} \textbf{VALID}: \texttt{SUM(c1 + 0) OVER (...)}
    \item \ding{55} \textbf{INVALID}: \texttt{(SUM(c1) OVER (...)) + 0}
\end{itemize}
...
Comparator trong MRUP Oracle là thành phần quan trọng thực hiện so sánh kết quả giữa $H(t_{union})$ và $H(t_1) \cup H(t_2)$ thông qua ba tầng: (1) Cardinality check đảm bảo số lượng hàng phù hợp, (2) MRUP normalization thực hiện sắp xếp partition-aware để chuẩn hóa thứ tự kết quả, và (3) Per-partition comparison thực hiện so sánh từng partition với type-aware comparison để xử lý chính xác các kiểu dữ liệu khác nhau trong SQLite.
