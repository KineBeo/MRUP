...
\begin{verbatim}
-- H(t1): 3 rows
-- {('Engineering', 90000, 1), ('Engineering', 80000, 2), ('Sales', 70000, 1)}
...
\begin{verbatim}
CREATE TABLE t1 (
    dept TEXT,           -- partition key, from {Engineering, Sales, Finance}
    salary INTEGER|REAL, -- orderable (66% INTEGER, 33% REAL)
    age INTEGER,         -- orderable (70% chance to include)
    c0 TYPE,             -- additional column (40% INT, 30% REAL, 30% TEXT)
    c1 TYPE,             -- additional column (nullable)
    ...                  -- 1-4 additional columns total
);

CREATE TABLE t2 (
    dept TEXT,           -- partition key, from {Marketing, HR, Operations}
...
\subsection{Tích hợp vào SQLancer}

MRUP Oracle được triển khai như một module trong SQLancer framework. Cấu trúc file bao gồm \texttt{SQLite3MRUPOracle.java} (main oracle logic), \texttt{SQLite3MRUPMutationOperator.java} (window spec mutations), \texttt{SQLite3MRUPCaseMutator.java} (CASE WHEN mutations), \texttt{SQLite3MRUPTestCaseLogger.java} (logging và reproduction scripts), và \texttt{SQLite3MRUPTablePairGenerator.java} (table pair generation).

\subsection{Workflow implementation}

Algorithm tổng quát của MRUP Oracle:

\begin{verbatim}
Input: Database schema
Output: Bug report (if found)

1. Generate table pair (t1, t2) with disjoint partitions
   Ensure: PARTITION_VALUES(t1) ∩ PARTITION_VALUES(t2) = ∅

2. Create union table: t_union = t1 ∪ t2

3. Generate window function query H
   Ensure: H satisfies constraints C0-C5

4. Apply mutations to H → H'
...
5. Execute queries:
   R1 = Execute(H', t1)
   R2 = Execute(H', t2)
   R_union = Execute(H', t_union)

6. Compare results using three-layer comparator:
   R_expected = R1 ∪ R2
   If Compare(R_union, R_expected) ≠ EQUAL:
       Report Bug with reproduction script
\end{verbatim}

\subsection{Ví dụ bug-revealing case}

Giả sử MRUP Oracle phát hiện một lỗi trong SQLite khi sử dụng CASE WHEN với window functions:
...
SELECT dept, salary,
       ...
       ELSE NULL END AS wf
FROM t1;
-- Expected: 2 rows  ✓ {('Engineering', 90000, 1), ('Engineering', 80000, 2)}
...
SELECT dept, salary,
       ...
       ELSE NULL END AS wf
FROM t2;
-- Expected: 2 rows  ✓ {('Marketing', 75000, 1), ('HR', 65000, 1)}
...
SELECT dept, salary,
       ...
       ELSE NULL END AS wf
FROM t_union;
-- Expected: 4 rows  ✗ Actual: 3 rows (bug detected!)
\end{verbatim}

MRUP Oracle phát hiện cardinality mismatch (3 rows thay vì 4 rows), báo lỗi và tạo reproduction script.
